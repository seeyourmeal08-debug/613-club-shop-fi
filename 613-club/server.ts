...
async function startServer() {
  ...
  app.post("/api/checkout", async (req, res) => {
    try {
      ...
      let session;
      try {
        // Attempt to create the session with your live promotion code pre-applied
        session = await stripe.checkout.sessions.create({
          ...
          discounts: [
            {
              promotion_code: "promo_1TgncwB91Q9ZMuPw885a0TT9"
            }
          ],
        ...
        console.warn("Could not pre-apply promotion code, falling back to allow_promotion_codes. Error:", discountErr.message);
        // Fallback option allowing manual entry if the specific promo code is missing or in Test Mode
        session = await stripe.checkout.sessions.create({
          payment_method_types: ["card"],
          line_items: lineItems,
          mode: "payment",
