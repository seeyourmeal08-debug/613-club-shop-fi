...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!cartItems || !Array.isArray(cartItems) || cartItems.length === 0) {
      return status(400).json({ error: "Le panier est vide." });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      shipping_options: [
        {
          shipping_rate: "shr_1TRfdCB91Q9ZMuPwUZHD9JZu", // Livraison à 9,95$
        },
        {
          shipping_rate: "shr_1TYx7DB91Q9ZMuPwLDzr8kuF", // Pickup chez Massalia Pizza (Gratuit)
        },
      ],
    });
